        ...
        <div class="card border border-light-subtle rounded-2xl p-4 p-md-5 bg-white">
            <form action="{{ route('owner.hostels.update', $hostel) }}" method="POST" enctype="multipart/form-data" class="d-flex flex-column gap-4">
                ...
                <div>
                    ...
                    <div class="row g-3">
                        @php
                            $associatedCoaching = old('coaching_centers', $hostel->coachingCenters->pluck('id')->toArray());
                            $coachingDistances = old('coaching_distances', $hostel->coachingCenters->pluck('pivot.distance_km', 'id')->toArray());
                        @endphp
                        @foreach($coachingCenters as $center)
                            <div class="col-12 col-md-6 col-lg-4">
                                <div class="p-3 rounded-xl border border-light-subtle d-flex flex-column gap-2" style="background-color: #fafbfd;">
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" name="coaching_centers[]" value="{{ $center->id }}" id="coaching_{{ $center->id }}" {{ in_array($center->id, $associatedCoaching) ? 'checked' : '' }}>
                                        <label class="form-check-label text-dark small fw-semibold" for="coaching_{{ $center->id }}">
                                            {{ $center->title }}
                                        </label>
                                    </div>
                                    <div class="input-group input-group-sm">
                                        <input type="number" step="0.01" min="0" name="coaching_distances[{{ $center->id }}]" value="{{ $coachingDistances[$center->id] ?? '' }}" class="form-control rounded-start-xl" placeholder="e.g. 0.5">
